...
from format_features import create_album_score_lookup_table, assign_value_redesigned
...
df, artist_lookup_table = assign_artist_features_inplace(df)
...
sub.to_csv(f"baseline3_predictionscoreRedesigned_StatifiedAlbum{mean_rmse:.4f}_{std_rmse:.4f}.csv", index=False, header=False)
