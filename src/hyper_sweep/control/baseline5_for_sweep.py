...
def perform_cv_lightgbm(df_train, chosen_features, params, early_stopping_rounds, n_folds):
    ...
    tmp_chosen_features = ["album_right", "istrack11", "no_artist", "no_composer", "freq_artist", "freq_composer", "year",
                       "month", "hour", "day", "len_of_songname",
                       "isRemix", "isOST", "isBeat", "isVersion", "isCover", "num_song_release_in_final_month",
                       "length", "genre", "track", "album_artist", "islyric", "album_artist_contain_artistname",
                       "len_album_name", "isRemixAlbum", "isOSTAlbum", "isSingleAlbum", "album_name_is_title_name",
    ...
    print("NFOLDS IS ", n_folds, "LENGTH OF WHOLE DATA", len(df_train), df_train.album_right.isnull().sum(), len(df_train.album_right.unique()))
    df_train.to_csv('/home/ben/zalo-hit-song-prediction/src/hyper_sweep/control/dataframe2.csv')
    print("DF WROTE TO FILE")
    ...
    for fold_, (trn_idx, val_idx) in enumerate(folds.split(df_train.values, df_train.album_right.values)):
        print("LENGTH OF FOLDS TRN/ VAL before entering CV", len(df_train.iloc[trn_idx][tmp_chosen_features]), len(df_train.iloc[val_idx][tmp_chosen_features]))
        print("SUM OF FOLDS before entering CV", len(df_train.iloc[trn_idx][tmp_chosen_features])+len(df_train.iloc[val_idx][tmp_chosen_features]))

    #print([i for i in folds.split(df_train.values, df_train.album_right.values)])
    ...
    for fold_, (trn_idx, val_idx) in enumerate(folds.split(df_train.values, df_train.album_right.values)):
        print('AT START OF CV', len(df_train.iloc[trn_idx][tmp_chosen_features]), len(df_train.iloc[val_idx][tmp_chosen_features]), tmp_chosen_features, len(df_train.album_right.unique()))
        print(df_train.iloc[trn_idx][tmp_chosen_features].values[0])
        ...
        print(len(df_train.iloc[trn_idx][chosen_features]), len(df_train.iloc[val_idx][chosen_features]), chosen_features)
